...
import Data.Foldable (class Foldable, intercalate, null, traverse_)
...
mkTabPanes ∷ ∀ m f1 f2 f3 f4 f5 f6 d. MonadEffect m ⇒ Foldable f1 ⇒ Foldable f2 ⇒ Foldable f3 ⇒ Foldable f4 ⇒ Foldable f5 ⇒ Foldable f6 ⇒
...
mkTabPanes details classNames = do
  tabs ← el "nav" ["class" /\ intercalate " " classNames.tabsContainer, "role" /\ "tablist"] []
  tabPanePairArray ← A.new
  contentPanes ← H.div ["class" /\ intercalate " " classNames.panesContainer] []
  traverse_ (\ detail → do
    hasChildren ← hasChildNodes tabs
    let paneId = "nav-" <> detail.label
        tabId = paneId <> "-tab"
    contentUi ← H.div [
      "class" /\ intercalate " " classNames.panes,
      "aria-labelledby" /\ tabId,
      "id" /\ paneId,
      "role" /\ "tabpanel",
      "tabindex" /\ "0"] [detail.content] # if hasChildren then C.hideM else C.showM
    appendChild contentUi $ toNode contentPanes
    tabUi ← el "button" [
      "class" /\ (intercalate " " classNames.allTabs <> (if hasChildren && not (null classNames.activeTab) then "" else " " <> intercalate " " classNames.activeTab)),
      ...
      "id" /\ tabId,
      "role" /\ "tab",
      "type" /\ "button"] [detail.tab] #
      onM "click" \ _ → do
        -- iterate through content panes
        traverse_ (\ (tab /\ pane) → do
          (
          let toggle f bs g = do
                traverse_ ((>>=) (classList tab) <<< flip f) classNames.activeTab
                _ ← setAttrs ["aria-selected" /\ bs] tab
                g pane
          in
          if unsafeRefEq pane contentUi
          then toggle D.add "true" C.show
